...
namespace CustomStrokeForge
{
    internal sealed class GraphicsEffectPool : IDisposable
    {
        ...
        public StrokeCustomEffect Rent(IGraphicsDevicesAndContext devices)
        {
            ...
            return new StrokeCustomEffect(devices);
        }

        public void Return(IGraphicsDevicesAndContext devices, StrokeCustomEffect effect)
        {
        ...
        private sealed class Bucket
        {
            ...
            public bool TryRent(out StrokeCustomEffect? effect)
            {
                lock (_lock)
                {
            ...
            public bool TryReturn(StrokeCustomEffect effect, int maxSize)
            {
                lock (_lock)
                {
        ...
        {
            public readonly StrokeCustomEffect Effect;
            public readonly long ReturnTimeTicks;

            public PooledEntry(StrokeCustomEffect effect)
            {
                Effect = effect;
